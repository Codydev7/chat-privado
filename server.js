...
// Health FIRST for Abasthan free tier
  ...
  ws.on('message', async (raw)=>{
    try{
      ...
      if(msg.type==='presence'){
        ...
        if(!newName) return;
        // If changing name, remove old
        if(currentUser && currentUser!==newName){
          prevUser=currentUser;
          clients.delete(prevUser);
          console.log(`[RENAME] ${prevUser} -> ${newName}`);
        }
        currentUser=newName;
        clients.set(currentUser, ws);
        console.log(`[PRESENCE] ${currentUser} total ${clients.size}`);
      ...
      if(msg.type==='admin-revoke-license'){
        ...
        console.log(`[ADMIN] Revoke license ${target} by ${currentUser}`);
        if(target==='ALL'){
          broadcast({type:'admin-revoke-license', target:'ALL'});
        } else {
          const tWs=clients.get(target);
          if(tWs) tWs.send(JSON.stringify({type:'admin-revoke-license', target:target}));
      ...
      // Relay types - including calls and remote audio
      const relayTypes=[
        ...
        'call-offer','call-answer','call-ice','call-reject','call-end','call-busy','call-audio-chunk','remote-audio-chunk','call-mute','admin-revoke-license'
      ];
