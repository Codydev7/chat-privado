...
app.post('/api/validate-key', (req,res)=>{
  ...
  // allow any key containing ADMIN as admin
  if(key.includes('ADMIN')) return res.json({valid:true, role:'admin'});
  const found = licenseKeys.find(k=>k.key===key);
  if(found) return res.json({valid:true, role:found.role, name:found.name});
  // fallback: accept any 3+ chars for backward compatibility if no keys enforced
  if(licenseKeys.length===0) return res.json({valid:true, role:'user'});
  // if keys exist, require exact match, but still allow legacy if key length >3? We'll enforce only if found or ADMIN
  // For now accept length >3 to not break existing
  if(key.length>=3) return res.json({valid:true, role:'user'});
...
const clients = new Map(); // username -> {ws, photo, avatar, name, ip, lastSeen}
let messageHistory=[];
let pinnedStore=[];
  ...
  ws.on('message', async (raw)=>{
    try{
      ...
      if(msg.type==='message'){
        ...
        // If temp message, keep expiresAt
      ...
      if(msg.type==='temp-toggle'){
        if(!msg.enabled){
          // Si desactiva, cancelar borrado en servidor también
          let canceled=0;
      ...
      if(msg.type==='profile-update'){
        // Broadcast photo update - ensures other user sees new photo instantly
      ...
      const relayTypes=[
        ...
        'call-offer','call-answer','call-ice','call-reject','call-end','call-busy','call-audio-chunk','remote-audio-chunk','call-mute','profile-update'
      ];
...
// Auto clean expired temp messages every 5 min
setInterval(()=>{
  const now=Date.now();
...
server.listen(PORT, HOST, ()=>{
  console.log(`LIVE on ${HOST}:${PORT} with IP tracking + pins + temp`);
});
