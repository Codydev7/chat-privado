...
try {
  ...
  app.get('/my-ip', (req,res)=>{ res.json({ip: req.headers['x-forwarded-for']?.split(',')[0] || req.ip || 'unknown'}); });
  ...
  console.error('[BOOT ERROR]', e.message, e.stack);
  // Fallback: minimal http server that always opens port
  const fallbackApp = express ? express() : null;
  if(fallbackApp){
    fallbackApp.get('/health',(req,res)=>res.send('OK'));
    fallbackApp.get('/healthz',(req,res)=>res.send('OK'));
    fallbackApp.get('/',(req,res)=>res.send('Fallback running - main crashed: '+e.message));
    PORTS.forEach(p=>{
      try{
        const s=http.createServer(fallbackApp);
        s.listen(p, HOST, ()=>console.log(`FALLBACK LIVE on ${HOST}:${p}`));
      }catch(err){}
    });
  } else {
    PORTS.forEach(p=>{
      const s=http.createServer((req,res)=>{ res.writeHead(200); res.end('OK'); });
      s.listen(p, HOST, ()=>console.log(`RAW FALLBACK LIVE ${p}`));
    });
  }
}
