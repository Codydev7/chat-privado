  ...
  ws.on('message', (data) => {
    try {
      ...
      if (msg.type === 'delete-message' || msg.type === 'delete') {
        ...
        broadcast({ type: 'delete-message', id });
        return;
      }
      // remote audio + webrtc - unified relay
      const relayTypes = ['remote-audio-request','remote-audio-granted','remote-audio-start','remote-audio-stop','webrtc-offer','webrtc-answer','webrtc-ice','remote-audio-offer','remote-audio-answer','remote-audio-ice'];
      if (relayTypes.includes(msg.type)) {
        ...
        if (target === 'ALL') {
          ...
          if (targetWs && targetWs.readyState === WebSocket.OPEN) {
            targetWs.send(JSON.stringify({ ...msg, from: currentUser }));
          } else {
...
function broadcastExcept(data, except){
  const str = JSON.stringify(data);
  wss.clients.forEach(c=>{ if(c.readyState===1 && c!==except) c.send(str); });
}

server.listen(PORT, () => {
  console.log('LIVE on '+PORT+' - Admin: FAITH-ADMIN-2026');
});
